            ...
            <h2 className="text-2xl">Title</h2>
                ...
                <div className="mt-2 grid gap-2 grid-cols-3 ">
                    {images.length > 0 && images.map((image,index) => (
                        <div key={index}>
                            <img className = "rounded-2xl" src={`http://localhost:4000/uploads/${image}`}/>
                        </div>
                    ))}
                    <label className="cursor-pointer items-center gap-1 justify-center border bg-transparent rounded-2xl p-2 text-2xl text-gray-600">
